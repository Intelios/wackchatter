 ...
 *
 * `reasoning_effort` is Low by default: extraction is structured output, and a reasoning
 * model will otherwise think its way through the entire output allowance and never emit
 * the JSON. Auto resolves to "send nothing" for endpoints that reject the field.
 */
 ...
 *
 * A length cut with text is a real truncation. A length cut with *no* text is the
 * reasoning-model failure: the whole budget went to thinking (proved either by counted
 * reasoning tokens or by reasoning text, since not every provider reports the details
 * object), and "increase the allowance" would be advice the field's cap makes
 * unachievable — the answer is less reasoning, not more tokens.
 */
export function nexusRequestError(result: NexusRequestResult): string | null {
  if (result.finishReason === 'length') {
    ...
    if (!result.content.trim() && reasoned)
      return 'The memory model spent its entire output budget on reasoning and produced no memories. Lower the reasoning effort in Nexus settings, or choose a non-reasoning model.';
