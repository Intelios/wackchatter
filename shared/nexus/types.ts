...
export const NEXUS_PROMPT = `You maintain a faithful memory of an ongoing story. Extract short, standalone facts, meaningful events, current situations, promises and unresolved goals. A small fact usually needs one sentence. Preserve who said or believed something: a suspicion, intention or reported claim is not established truth. Never infer unstated facts. Profiles explain identities but are not evidence for new memories. Record private knowledge as private, not as something everyone knows.
Use the supplied existing IDs when the evidence clearly identifies the same entity. Equal names alone do not prove identity. Add aliases only when explicitly established. Preserve exact names. Give facts useful search cues and connect people, places, objects and events only where supported by the transcript. Update clear changes by revising an existing generated record, preserving its identity; do not combine different predicates (hometown and current residence differ). Mark unresolved contradictions as conflict and name the conflicting record IDs. Leave manually edited, disabled and deleted records alone. Facts may overlap in source messages. Do not wait for a scene to finish. Never mention this application, extraction, or message numbering in memory text.`;
